...
export function ShelfSection() {
  ...
  return <section className="shelf-page shelf-inline" id="shelf" aria-labelledby="shelf-title" data-scroll>
    ...
    <section className="shelf-beans section-shell" id="beans" aria-labelledby="beans-heading"><div className="shelf-section-top"><div><p className="eyebrow">The bean collection</p><h2 id="beans-heading">A cup for <em>your kind of day.</em></h2></div><button type="button" className="shelf-text-button" onClick={() => setView("roasters")}>Meet all 9 sellers <Arrow /></button></div><p className="shelf-notes">Chocolate, honey, fruit: these are tasting impressions, not added ingredients. Our featured selection is plain coffee, for brewing without milk or sugar.</p>
      ...
      <div className="bean-grid" key={`${roast}-${seller}`}>{visible.map(bean => <article className={`bean-card bean-card-${bean.tone}`} key={bean.id} data-scroll data-reveal>
        <a className="bean-photo" href={bean.url} target="_blank" rel="noreferrer" aria-label={`Explore ${bean.name} at ${bean.seller} (opens a new tab)`}>
          <div className="bean-photo-drift"><img src={bean.image} alt={`${bean.name} coffee packaging by ${bean.seller}`} loading="lazy" decoding="async" width="900" height="900" /></div>
          <span className="bean-number" aria-hidden="true">{String(beans.indexOf(bean) + 1).padStart(2, '0')}</span><span className="bean-photo-arrow"><Arrow /></span>
          <span className="bean-roast-label">{bean.roast} roast</span>
        </a>
        <div className="bean-copy"><p className="eyebrow bean-seller">{bean.seller}</p><h3>{bean.name}</h3><p className="shelf-caption bean-origin">{bean.origin}</p><p className="bean-notes">{bean.notes}</p><p className="bean-thought">{bean.thought}</p></div>
        <div className="bean-card-bottom"><span>{bean.process}</span><a href={bean.url} target="_blank" rel="noreferrer">Shop the roaster <Arrow /><span className="sr-only">: {bean.name} at {bean.seller} (opens a new tab)</span></a></div>
      </article>)}</div>
      ...
      <p className="shelf-caption">Product images from the linked roasters; packaging may vary. Roast and tasting descriptions come from the roasters; brew pairings are our editorial starting points, not personal tasting reviews. External shop links open a new tab. Availability, ingredients, grind options, delivery, and prices are confirmed on the seller’s site.</p>
    </section>
    </>}
